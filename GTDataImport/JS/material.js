...
function json_to_array(datas) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var OnSaleName = GetErrorImage(data[i].OnSaleName);
        isok = judge(OnSaleName);
        if (isok == false) {
            result = false;
        }

        var OnSaleUnit = CheckArray(unitArr, data[i].OnSaleUnit);
        isok = judge(OnSaleUnit);
        if (isok == false) {
            result = false;
        }

        var OnSaleType = CheckArray(goodsTypeArr, data[i].OnSaleType);
        isok = judge(OnSaleType);
        if (isok == false) {
            result = false;
        }

        var OnSalePrice = GetIntValue(data[i].OnSalePrice, 0);
        isok = judge(OnSalePrice);
        if (isok == false) {
            result = false;
        }

        var proxy = data[i].IsOnSale;
        if (proxy == "true") {
            proxy = "是";
        }
        else if (proxy == "false") {
